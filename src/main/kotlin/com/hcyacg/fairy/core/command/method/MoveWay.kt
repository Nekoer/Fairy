...
class MoveWay: GameCommandService, DependenceService() {
    override fun group(sender: Long, group: Long, message: String): String {
        ...
        account?.let {
            ...
            if (move.move){
                sb.append("您已从(${move.before.id})${move.before.name}(${Architecture.getArchitectureById(move.before.architectureId).value})移动到(${move.after.id})${move.after.name}(${Architecture.getArchitectureById(move.after.architectureId).value})")
            }else {
